...
|------|-------------------|-------|---------|
| `test_config.py` | `app/config.py` | 10 | Defaults load without `.env`; `origins_list` parses correctly |
| `test_models.py` | `app/models.py` | 13 | `ChatRequest` validation, `ChatResponse` serialisation, `MetricsResponse` shape |
...
| `test_policy_guard.py` | `app/services/policy_guard.py` | 46 | 31+ blocked patterns, false-positive checks, `build_messages()` structure |
...
| `test_e2e.py` | All modules composed | 6+4 skip | Full lifecycle, policy sweep, input validation, metrics consistency, CORS/cascade TODO |
